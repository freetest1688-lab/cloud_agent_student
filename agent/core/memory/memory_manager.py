"""Unified memory manager that coordinates short-term and long-term storage.

MemoryManager is the single entry point for all memory operations in the agent
framework. It delegates to:

- :class:`ShortTermMemory`     – Redis, TTL-based recent conversation history
- :class:`LongTermMemory`      – Milvus, vector-based user preferences / facts
- :class:`PreferenceExtractor` – LLM-based extraction (invoked at session end)

Both storage backends degrade gracefully when their services are unavailable.

Session lifecycle
-----------------
1. **New session, first query** – call ``load_preferences(user_id)`` to fetch
   all stored preferences from Milvus (cache on the caller side).
2. **Each query turn** – call ``save_conversation(user_id, session_id, msgs)``
   to persist recent messages to Redis.
3. **Session end** – call ``finalize_session(user_id, session_id, llm)``
   to extract preferences via LLM, save to Milvus, and clear Redis.
"""
...
class MemoryManager:
    """Coordinates short-term Redis memory and long-term Milvus memory.

    Args:
        redis_url: Redis connection URL.
        redis_ttl: Short-term memory TTL in seconds (default 30 minutes).
        milvus_host: Milvus server hostname.
        milvus_port: Milvus server port.
        milvus_api_key: Optional Milvus authentication token.
        embedding_api_key: API key used for Milvus embeddings.
    ...
    Example::
        ...
        # New session – load once and cache:
    ...
    async def load_preferences(
        self,
        user_id: str,
        query: str = "user preferences habits personality traits",
        top_k: int = 3,
    ) -> list[str]:
        ...
        Args:
            ...
            query: Semantic search query (use the user's first question
                   for best relevance).  Defaults to a broad phrase
                   that covers all preference types.
            top_k: Maximum number of preferences to return (default 3).

        Returns:
            List of preference strings (empty if Milvus is unavailable).
        """
